...
验证状态：Python 检查投影端点、反函数、眼空间与欧氏距离差别、比较/写入模型及 UNorm 误差估算。Shader 未在 Unity 编译、运行或抓帧。已核对本地 `Manual/SL-ZTest.html`、`SL-ZWrite.html`、`SL-PlatformDifferences.html`、`ScriptReference/SystemInfo-usesReversedZBuffer.html`，根目录为 `E:\Claude Skills\学习仓库\09_Unity官方文档\Documentation\en`。
