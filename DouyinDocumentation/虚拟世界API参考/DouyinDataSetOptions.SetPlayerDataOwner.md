SetPlayerDataOwner(string ownerID)
# 描述
设置数据关联的玩家（仅用于检测），平台侧即可检测到该玩家因多房间同时写入而导致的数据回档异常。
# 参数
| 参数 | 类型 | 描述 |
| --- | --- | --- |
| ownerID | string | 数据所有者的玩家 playerOpenID |
# 代码示例
```Lua
...
function ServerDataStoreManager.SetData(storeName, scopeName, dataKey, dataValue, callback, playerOpenID)
    ...
    DATASTORE(function()
        ...
        options:SetPlayerDataOwner(playerOpenID)
