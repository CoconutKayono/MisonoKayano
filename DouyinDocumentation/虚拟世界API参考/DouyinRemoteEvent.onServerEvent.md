public UnityEvent<DouyinPlayer, object[]> onServerEvent
# 描述
当客户端调用 `FireServer` 方法时，在服务端触发此事件。您可以在服务端通过 `AddListener` 方法来监听此事件，并在回调函数中处理相应的业务逻辑。
回调函数会接收到以下两个参数：

* `player: DouyinPlayer`：触发该事件的客户端所对应的玩家对象。
* `args: object[]`：客户端调用 `FireServer` 时传递的参数列表。
