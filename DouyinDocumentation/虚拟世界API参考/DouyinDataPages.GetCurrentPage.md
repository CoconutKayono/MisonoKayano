...
# 描述
获取当前页面的所有数据。
返回的数据存储在表中，表中数据的实际类型取决于 DouoyinDataPages 实例的来源。

* 如果 DouoyinDataPages 实例是通过使用 DouoyinDataStore.ListVersion API 创建的，则表中存储的数据类型是 DouoyinDataInfo。
* 如果 DouoyinDataPages 实例是通过使用 DouoyinDataStore.ListKeys API 创建的，则表中存储的数据类型是字符串。
* 如果 DouoyinDataPages 实例是通过使用 DouoyinDataStore.ListValues API 创建的，则表中存储的数据类型是 ParaDataInfo。
* 如果 DouoyinDataPages 实例是通过使用 DouoyinOrderedDataStore.ListSortedData API 创建的，则表中存储的数据类型是 DouoyinOrderedDataInfo。
