public void FireClient(DouyinPlayer player, object[] args)
# 参数
| 参数名称 | 参数描述 |
| --- | --- |
| player | 指定事件的目标客户端 `DouyinPlayer` 对象 |
| args | 用于将服务器的数据传递给目标客户端的参数 |
# 描述
服务器端调用，用于向指定的客户端触发一个远程事件。开发者通过 `player` 参数来确定接收事件的目标客户端，并可以通过 `args` 将任意数量的参数从服务器传递到该客户端。要成功接收并处理此事件，目标客户端必须已通过 `OnClientEvent` 注册了相应的监听器（回调函数）。
# 代码示例
客户端
```Lua
