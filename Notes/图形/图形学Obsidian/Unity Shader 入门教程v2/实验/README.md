...
这里集中说明 A 阶段 Unity 实验；B01—B04 见 [Shader 执行与图元实验](B01-B04实验说明.md)，B05—B08 见 [B 阶段检查点](B阶段检查点.md)，C01—C04 见 [深度优化、合成与附件实验](C01-C04实验说明.md)，C05—C08 与阶段汇总见 [C 阶段检查点](C阶段检查点.md)。后续实践以 Unity 操作、预期画面与排查方法为主，不要求使用 Python。
