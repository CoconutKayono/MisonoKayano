...
> [!IMPORTANT]
> 内置渲染管线（Built-In Render Pipeline）已弃用，并将在未来版本中废止。在整个 Unity 6.7 LTS 生命周期内，它仍会获得支持，包括错误修复和维护。有关迁移的更多信息，请参阅 [[05-使用通用渲染管线/04-开始使用 URP/02-安装和升级 URP/02-从内置渲染管线升级到 URP/00-从内置渲染管线升级到 URP]] 和 [[03-选择渲染管线/02-渲染管线功能比较参考]]。
...
通用渲染管线（URP）还有一个 [[05-使用通用渲染管线/04-开始使用 URP/01-通用渲染管线基础知识/02-URP 中的渲染路径/04-URP 中的前向渲染路径|Forward+ 渲染路径]]，它类似于前向渲染路径，但不限制每个 GameObject 可使用的光源数量。

> **注意：**内置渲染管线还有一个 [[07-使用内置渲染管线/03-内置渲染管线中的渲染路径/04-内置渲染管线中的旧版顶点光照渲染路径|Legacy Vertex Lit]] 渲染路径，它是前向渲染路径的子集。
...
- [[05-使用通用渲染管线/04-开始使用 URP/01-通用渲染管线基础知识/02-URP 中的渲染路径/00-URP 中的渲染路径]]
...
- 上一页：[[03-选择渲染管线/04-更改或检测激活的渲染管线]]
- 目录：[[00-渲染管线]]
- 下一页：[[05-使用通用渲染管线/00-使用通用渲染管线]]
