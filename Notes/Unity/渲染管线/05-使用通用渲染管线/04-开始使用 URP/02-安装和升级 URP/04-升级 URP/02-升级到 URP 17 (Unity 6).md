...
> 原文：[Upgrade to URP 17 (Unity 6.0)](https://docs.unity3d.com/6000.7/Documentation/Manual/urp/upgrade-guide-unity-6.html)
...
URP 17 引入了[[../../../07-在 URP 中自定义渲染和后期处理/05-URP 中的渲染图系统/01-URP 中的渲染图系统简介]]系统，自定义渲染通道的编写方式有重大变化。

如果项目包含自定义渲染通道，请使用渲染图 API 重写通道。有关渲染图的更多信息，请参阅[[../../../07-在 URP 中自定义渲染和后期处理/05-URP 中的渲染图系统/00-URP 中的渲染图系统]]部分。

如需查看使用渲染图 API 的 Scriptable Renderer Feature（包含渲染通道）的完整示例，请执行以下步骤：

1. 导入 URP 渲染图示例。有关更多信息，请参阅[[../01-创建 URP 项目/03-在 URP 中导入包示例]]。
2. 在**项目 (Project)** 窗口中，转到**示例 (Samples)** > **通用渲染管线 (Universal Render Pipeline)** > **17.0.0** > **URP RenderGraph Samples**。

**注意：**Unity 不再开发或改进不使用[[../../../07-在 URP 中自定义渲染和后期处理/05-URP 中的渲染图系统/00-URP 中的渲染图系统]] 的渲染路径。
...
为了更好地了解渲染图的实现，请使用[[../../../07-在 URP 中自定义渲染和后期处理/05-URP 中的渲染图系统/08-在 URP 中分析渲染图]]。
...
- 上一页：[[01-升级到 URP 17.1（Unity 6.1）]]
- 目录：[[00-升级 URP]]
- 下一页：[[03-升级到 URP 16 (Unity 2023.2)]]
