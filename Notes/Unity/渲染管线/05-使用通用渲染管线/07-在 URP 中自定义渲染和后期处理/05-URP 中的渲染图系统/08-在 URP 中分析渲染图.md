...
![渲染图查看器 (Render Graph Viewer) 示例](图片/render-graph-viewer.png)
...
2. **主光源阴影贴图 (Main Light Shadowmap)** 渲染通道将纹理创建为全局纹理，并且对其具有只读访问权限。有关全局纹理的更多信息，请参阅[[04-URP 中的 Render Graph 系统中的纹理/03-在 URP 中的渲染通道之间传输纹理/00-在 URP 中的渲染通道之间传输纹理]]。  **主光源阴影贴图 (Main Light Shadowmap)** 下方的蓝色合并栏表示 URP 将**主光源阴影贴图 (Main Light Shadowmap)**、**附加光源阴影贴图 (Additional Lights Shadowmap)** 和 **SetupCameraProperties** 合并到单个渲染通道。
3. 接下来的五个渲染通道无法访问纹理。
...
- 上一页：[[07-在 URP 的渲染图系统中的计算着色器/02-在 URP 中为计算着色器创建输入数据]]
- 目录：[[00-URP 中的渲染图系统]]
- 下一页：[[09-优化渲染图]]
