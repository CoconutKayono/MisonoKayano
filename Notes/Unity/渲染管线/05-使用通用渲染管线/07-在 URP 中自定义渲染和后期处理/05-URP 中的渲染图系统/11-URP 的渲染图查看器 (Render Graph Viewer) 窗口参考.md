...
| 灰色 | 渲染通道无法访问资源。 |
| 地球图标 | 渲染通道将纹理设置为全局资源。如果地球图标具有灰色背景，则资源作为 `TextureHandle` 对象导入到渲染图中，并且通道使用 `SetGlobalTextureAfterPass` API。请参阅[[04-URP 中的 Render Graph 系统中的纹理/02-在 URP 中的渲染图形系统中创建纹理]]和[[04-URP 中的 Render Graph 系统中的纹理/03-在 URP 中的渲染通道之间传输纹理/00-在 URP 中的渲染通道之间传输纹理]]。 |
...
| Resource name | 资源名称。 |
| Imported resource | 如果导入了资源，则显示向左箭头。请参阅[[04-URP 中的 Render Graph 系统中的纹理/03-在 URP 中的渲染通道之间传输纹理/03-将纹理导入 URP 中的渲染图系统]]。 |

![渲染图查看器 (Render Graph Viewer) 图标](图片/render-graph-viewer-icons.png)
...
| **图标** | **描述** |
| --- | --- |
| ![纹理图标](图片/render-graph-viewer-icon-texture.png) | 纹理。 |
| ![加速结构图标](图片/render-graph-viewer-icon-acceleration-structure.png) | 加速结构。 |
| ![缓冲区图标](图片/render-graph-viewer-icon-buffer.png) | 缓冲区。 |
...
- 目录：[[00-URP 中的渲染图系统]]
- 下一页：[[../06-在 URP 中将可编程渲染通道添加到帧渲染循环/00-在 URP 中将可编程渲染通道添加到帧渲染循环]]
