...
某些纹理可能不存在于帧数据中，具体取决于用于将自定义渲染通道插入到 URP 帧渲染循环中的注入点。有关何时存在哪些纹理的更多信息，请参阅[[../../06-在 URP 中将可编程渲染通道添加到帧渲染循环/04-URP 的注入点参考]]。
...
然后，就可以读取和写入纹理了。请参阅[[../04-URP 中的 Render Graph 系统中的纹理/01-在 URP 中的渲染通道中使用纹理]]。
...
有关完整示例，请参阅[[../../../04-开始使用 URP/02-安装和升级 URP/01-创建 URP 项目/03-在 URP 中导入包示例]]名为 **TextureReference w. FrameData** 的示例。
...
- [[../../../04-开始使用 URP/01-通用渲染管线基础知识/02-URP 中的渲染路径/05-URP 中的延迟渲染路径/00-URP 中的延迟渲染路径]]
