...
| [[04-URP 中的 Render Graph 系统中的纹理/00-URP 中的 Render Graph 系统中的纹理]] | 在渲染通道中访问和使用纹理以及如何执行__ blit__“位块传输 (Bit Block Transfer)”的简写。blit 操作是将数据块从内存中的一个位置传输到另一个位置的过程。 <br><br>执行 Blit 操作。有关 Blit 术语，请参阅 [Glossary](https://docs.unity3d.com/6000.7/Documentation/Manual/Glossary.html#blit)。 &#124; |
| [[05-URP 中的渲染图系统中的帧数据/00-URP 中的渲染图系统中的帧数据]] | 获取 URP 为当前帧创建的纹理并在渲染通道中使用它们。 |
| [[06-在 URP 中的渲染图形系统中绘制对象]] | 使用 `RendererList` API 在渲染图系统中绘制对象。 |
| [[07-在 URP 的渲染图系统中的计算着色器/00-在 URP 的渲染图系统中的计算着色器]] | 创建一个运行计算着色器的渲染通道。 |
...
| [[04-URP 中的 Render Graph 系统中的纹理/00-URP 中的 Render Graph 系统中的纹理]] | Textures in the Render Graph system in URP |
| [[05-URP 中的渲染图系统中的帧数据/00-URP 中的渲染图系统中的帧数据]] | Frame data in the render graph system in URP |
| [[06-在 URP 中的渲染图形系统中绘制对象]] | Draw objects in the render graph system in URP |
| [[07-在 URP 的渲染图系统中的计算着色器/00-在 URP 的渲染图系统中的计算着色器]] | Compute shaders in the render graph system in URP |
