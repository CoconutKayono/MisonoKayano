...
![打开存在编译错误的项目时，Enter Safe Mode? 对话框会询问是否进入 Safe Mode](图片/SafeModeDialog.png)
...
![Safe Mode 中的 Unity Editor](图片/SafeModeEditor.png)
...
![Safe Mode 中显示可用 Asset 创建选项的受限 Editor 菜单](图片/SafeModeLimitedMenu.png)
...
![Project 窗口为不可选择的 Asset 显示通用图标](图片/SafeModeProjectWindow.png)

**Create (+)** 菜单按钮处于禁用状态，Project 窗口的上下文菜单也只提供一组精简选项。

![Safe Mode 中 Project 窗口内已禁用的 Create (+) 菜单按钮](图片/SafeModeCreateButton.png)
...
![Safe Mode 工具栏中的 Exit Safe Mode 按钮](图片/SafeModeExitButton.png)
...
- 上一页：[[06-Roslyn分析器和源生成器/05-Roslyn Analyzer和Source Generator的Additional Files]]
- 目录：[[00-调试和诊断]]
- 下一页：[[../00-脚本]]
