...
要从 Managed C# Code 访问 Unmanaged Function，需要声明一个名称相同、返回类型和参数类型兼容的 C# `static extern` Function，然后像调用其他 Method 一样调用它。完整的声明规则和示例请参阅 [[02-调用函数/00-调用函数]]。
...
更多信息请参阅 [[03-在托管和非托管代码间传递数据/00-传递数据]]。
