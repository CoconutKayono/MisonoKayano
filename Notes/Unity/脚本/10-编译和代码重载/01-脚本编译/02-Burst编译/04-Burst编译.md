...
将 C# 代码中兼容的部分编译为高度优化的 Native CPU Code。

| 主题 | 说明 |
| --- | --- |
| [[01-Burst简介]] | 了解 Burst 编译器的基础知识，以及它在 Unity 编译生态系统中的作用。 |
| [[02-Burst入门]] | 通过创建第一个简单的 Burst 编译示例代码开始使用 Burst。 |
| [[06-CSharp语言支持/00-CSharp语言支持]] | 查看 C# 语言中属于 Burst 可编译的高性能 C# 子集的元素。 |
| [[03-配置Burst编译/00-配置Burst编译]] | 定义 Burst 在不同上下文中编译的内容和方式，标记代码进行 Burst 编译，并配置编译过程的各个方面；其中包括 [[03-配置Burst编译/05-Play Mode中的Burst编译]]。 |
| [[07-Burst Intrinsics/00-Burst Intrinsics]] | 如果编写单指令、多数据（SIMD）汇编代码，使用底层 Intrinsics 从 Burst 中获得额外性能。 |
| [[13-Burst Editor窗口参考]] | 使用 Unity Editor 中的 Burst 菜单和 Burst Inspector 窗口配置 Burst 选项，并检查项目中可由 Burst 编译的 Job。 |
| [[09-平台构建支持参考]] | 使用适当的平台专用工具链，在不同平台上构建由 Burst 编译的项目代码。 |
| [[08-Burst AOT设置参考]] | 为目标平台和架构配置 Burst 的提前（AOT）编译设置。 |
| [[05-Burst优化/00-Burst优化]] | 通过调试和性能分析识别 Burst 编译代码中的错误或瓶颈，并配置各种选项来优化性能。 |
| [[15-Burst Modding支持]] | 将由 Burst 编译的代码作为附加库包含在 Mod 中。 |
