...
可以使用 TestPlayerBuildModifier 将 BuildOptions 设置为把 Player 构建到特定位置，但不运行它。结合 [[../../03-编写测试/01-测试前后的操作/02-构建时设置和清理#prebuildsetup-and-postbuildcleanup|PostBuildCleanup]]，可以在构建完成后自动退出 Editor。下面的示例演示了这一过程：
...
- 下一页：[[../../05-Unity Test Framework学习材料/00-Unity Test Framework学习材料]]
