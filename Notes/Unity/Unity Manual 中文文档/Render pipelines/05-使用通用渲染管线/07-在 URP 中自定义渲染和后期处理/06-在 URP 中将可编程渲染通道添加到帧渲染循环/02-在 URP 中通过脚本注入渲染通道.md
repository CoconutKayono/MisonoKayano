...
5. 选择**播放 (Play)**。每次 Unity 触发 `beginCameraRendering` 事件时，Unity 都会在控制台窗口中打印来自脚本的消息。  ![Unity 在控制台中打印日志消息。](log-message-in-console.png) Unity 在控制台中打印日志消息。
6. 要引发对 `OnDisable()` 方法的调用：在播放模式下，选择 Example Cube 并清除脚本组件标题旁边的复选框。Unity 会取消订阅 `RenderPipelineManager.beginCameraRendering` 事件中的 `WriteLogMessage`，并停止在控制台窗口中打印消息。  ![停用脚本组件。清除脚本组件标题旁边的复选框。](deactivate-script-component.png) 停用脚本组件。清除脚本组件标题旁边的复选框。
