...
| [[03-将纹理导入 URP 中的渲染图系统]] | 要在 Render Graph 系统渲染通道中创建或使用渲染纹理，请使用 `RTHandle` API。 |
| [[01-在 URP 中的渲染通道中使用纹理]] | 要允许渲染通道读取或写入纹理，请使用 Render Graph 系统 API 将纹理设置为输入或输出。 |
| [[00-在 URP 中的渲染通道之间传输纹理]] | 将纹理设置为全局纹理，或将纹理添加到帧数据。 |
...
| [[00-在 URP 中的渲染通道之间传输纹理]] | Use a texture in multiple render passes in URP |
