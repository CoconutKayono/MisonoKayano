...
如需了解如何将针对内置渲染管线项目制作的资源转换为与 URP 兼容的资源，请参阅[[06-使用渲染管线转换器转换资源]]页面。

## 从 URP 15 或 URP 16（Unity 2023.1、Unity 2023.2）升级

### 渲染图系统

URP 17 引入了[[01-URP 中的渲染图系统简介]]系统，自定义渲染通道的编写方式有重大变化。

如果项目包含自定义渲染通道，请使用渲染图 API 重写通道。有关渲染图的更多信息，请参阅[[00-URP 中的渲染图系统]]部分。

如需查看使用渲染图 API 的 Scriptable Renderer Feature（包含渲染通道）的完整示例，请执行以下步骤：

1. 导入 URP 渲染图示例。有关更多信息，请参阅[[03-在 URP 中导入包示例]]。
...
为了更好地了解渲染图的实现，请使用[[08-在 URP 中分析渲染图]]。
...
- URP 12 及更高版本实现了[[06-使用渲染管线转换器转换资源]]功能。此功能取代了以前在**编辑 (Edit) > 渲染管道 (Render Pipeline) > 通用渲染管道 (Universal Render Pipeline) > 升级 (Upgrade)…** 中提供的资源升级功能
