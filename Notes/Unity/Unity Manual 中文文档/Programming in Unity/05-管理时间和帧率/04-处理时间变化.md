...
![在发生延迟的第 8 帧，delta time 报告默认最大值 0.333 秒，而 unscaled delta time 报告上一帧以来实际经过的 1.016 秒](time-delta-unscaled.png)
...
![发生延迟前，time 和 unscaled time 相等并以相同速率增加；延迟帧使二者产生差异](time-and-unscaled.png)

同理，`Time.unscaledTime`（**b**）因为加入了未截断的值而增加了约 1 秒，而 `Time.time`（**c**）只增加了被截断的值。`Time.time` 不会追上真实时间，而是表现得像延迟只持续了 `Time.maximumDeltaTime`。

![delta time 在发生延迟的第 8 帧显著增大，而 smooth delta time 会将增量分散到后续帧并逐渐下降](time-delta-smoothed.png)
