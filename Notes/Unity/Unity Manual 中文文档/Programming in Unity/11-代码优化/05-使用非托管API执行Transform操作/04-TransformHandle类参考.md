...
- [`TransformHandle.DirectChildrenEnumerator`](https://docs.unity3d.com/6000.7/Documentation/ScriptReference/TransformHandle.DirectChildrenEnumerator.html) 是一个遍历 Transform 直接子节点的枚举器。
...
- [`TransformHandle.SubhierarchyEnumerator`](https://docs.unity3d.com/6000.7/Documentation/ScriptReference/TransformHandle.SubhierarchyEnumerator.html) 是一个枚举器，会产生一个 `TransformHandle` 实例及其所有后代。首先产生 `TransformHandle` 实例本身，然后按深度优先顺序产生其后代。
...
- **TransformHandle**：`transformHandle.SetParent(TransformHandle.None)`
  ...
  替代声明：`TransformHandle h = transformHandle; h.parent = TransformHandle.None;`
