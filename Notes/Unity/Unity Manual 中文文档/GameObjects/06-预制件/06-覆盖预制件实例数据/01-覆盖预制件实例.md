...
![Prefab Instance Inspector，其中添加的 Box Collider Component 是一个 Override，其图标上以加号徽标标示；Health 脚本中的 Max Health 属性也有 Override，以粗体和属性左侧的蓝线标示。](Unity/Unity%20Manual%20中文文档/GameObjects/06-预制件/06-覆盖预制件实例数据/图片/prefab-instance-overrides.png)

在 Hierarchy 中，添加的对象也会在其图标上显示加号徽标。

![Hierarchy 窗口显示带有子 GameObject Banana 的 Prefab Instance，Banana 是一个 Override。](Unity/Unity%20Manual%20中文文档/GameObjects/06-预制件/06-覆盖预制件实例数据/图片/prefab-instance-hierarchy-override.png)
...
![Prefab Instance 的 Inspector 窗口，其中已选择 Override 菜单。](Unity/Unity%20Manual%20中文文档/GameObjects/06-预制件/06-覆盖预制件实例数据/图片/prefab-instance-inspector.png)
