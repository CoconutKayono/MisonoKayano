...
![向上旋转 Camera 拍摄楼顶会使垂直线汇聚](LensShift_VRot.png)

如果改为向上移动 lens，而不是旋转 Camera，就可以改变图像构图，将楼顶纳入画面，同时保持平行线笔直。

![沿 Y 轴移动 lens 会改变 focal center，但保持垂直线笔直](LensShift_VShift.png)

同样，也可以使用 horizontal Lens Shift 拍摄宽物体，避免旋转 Camera 可能造成的 distortion。

![旋转 Camera 构图拍摄建筑会使水平线汇聚](LensShift_HRot.png)

![水平移动 lens 会重新构图，同时保持水平线笔直](LensShift_HShift.png)
...
![Y 轴 Lens Shift 前后的 Camera Frustum](Unity/Unity%20Manual%20中文文档/Camera/08-使用物理相机模拟真实相机/图片/ObliqueFrustum_LensShift.png)
