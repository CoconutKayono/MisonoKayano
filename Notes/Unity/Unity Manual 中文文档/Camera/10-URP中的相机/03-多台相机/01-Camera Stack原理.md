...
Camera Stack 由一台 [[01-相机渲染类型简介#Base Camera|Base Camera]] 和一台或多台 [[01-相机渲染类型简介#Overlay Camera|Overlay Camera]] 组成。Camera Stack 会使用 Stack 中所有 Camera 的合并输出覆盖 Base Camera 的输出。因此，凡是可以对 Base Camera 输出执行的操作，也可以对 Camera Stack 输出执行，例如将 Camera Stack 渲染到 render target 或应用 post-processing effect。
...
- [[02-设置Camera Stack]]
- [[01-Camera Inspector窗口参考|Camera Component 参考]]
